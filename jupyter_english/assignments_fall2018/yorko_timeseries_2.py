...
def feature_month_q1(X):
    return pd.DataFrame(X['time1'].dt.month.isin([1, 2, 3]))
# Month Q
def feature_month_q2(X):
    return pd.DataFrame(X['time1'].dt.month.isin([4, 5, 6]))
# Month Q
def feature_month_q3(X):
    return pd.DataFrame(X['time1'].dt.month.isin([7, 8, 9]))
# Month Q
def feature_month_q4(X):
    return pd.DataFrame(X['time1'].dt.month.isin([10, 11, 12]))
...
# interval1
def feature_interval_1(X):
    X['interval_1'] = (X['time2'] - X['time1']).astype('timedelta64[s]')
    return X[['interval_1']].fillna(0)
# interval2
def feature_interval_2(X):
    X['interval_2'] = (X['time3'] - X['time2']).astype('timedelta64[s]')
    return X[['interval_2']].fillna(0)
# interval3
def feature_interval_3(X):
    X['interval_3'] = (X['time4'] - X['time3']).astype('timedelta64[s]')
    return X[['interval_3']].fillna(0)
# interval4
def feature_interval_4(X):
    X['interval_4'] = (X['time5'] - X['time4']).astype('timedelta64[s]')
    return X[['interval_4']].fillna(0)

def feature_interval_5(X):
    X['interval_5'] = (X['time6'] - X['time5']).astype('timedelta64[s]')
    return X[['interval_5']].fillna(0)

def feature_interval_6(X):
    X['interval_6'] = (X['time7'] - X['time6']).astype('timedelta64[s]')
    return X[['interval_6']].fillna(0)

def feature_interval_7(X):
    X['interval_7'] = (X['time8'] - X['time7']).astype('timedelta64[s]')
    return X[['interval_7']].fillna(0)

def feature_interval_8(X):
    X['interval_8'] = (X['time9'] - X['time8']).astype('timedelta64[s]')
    return X[['interval_8']].fillna(0)

def feature_interval_9(X):
    X['interval_9'] = (X['time10'] - X['time9']).astype('timedelta64[s]')
    return X[['interval_9']].fillna(0)

def feature_is_in_top_site(X):
    top_sites = pd.Series(train_df[sites].values.flatten()).value_counts().\
        sort_values(ascending=False).head(10)

    return pd.DataFrame(X[sites] == top_sites[0])
...
#         ])),
        ...
        ('interval_1', Pipeline([
            ('extract', FunctionTransformer(feature_interval_1, validate=False)),
            ('scale', StandardScaler()),
            ('shape', ShapeSaver())
         ])),
        ('interval_2', Pipeline([
            ('extract', FunctionTransformer(feature_interval_2, validate=False)),
            ('scale', StandardScaler()),
            ('shape', ShapeSaver())
         ])),
        ('interval_3', Pipeline([
            ('extract', FunctionTransformer(feature_interval_3, validate=False)),
            ('scale', StandardScaler()),
            ('shape', ShapeSaver())
         ])),
        ('interval_4', Pipeline([
            ('extract', FunctionTransformer(feature_interval_4, validate=False)),
            ('scale', StandardScaler()),
            ('shape', ShapeSaver())
         ])),
        ('interval_5', Pipeline([
            ('extract', FunctionTransformer(feature_interval_5, validate=False)),
            ('scale', StandardScaler()),
            ('shape', ShapeSaver())
         ])),
        ('interval_6', Pipeline([
            ('extract', FunctionTransformer(feature_interval_6, validate=False)),
            ('scale', StandardScaler()),
            ('shape', ShapeSaver())
         ])),
        ('interval_7', Pipeline([
            ('extract', FunctionTransformer(feature_interval_7, validate=False)),
            ('scale', StandardScaler()),
            ('shape', ShapeSaver())
         ])),
        ('interval_8', Pipeline([
            ('extract', FunctionTransformer(feature_interval_8, validate=False)),
            ('scale', StandardScaler()),
            ('shape', ShapeSaver())
         ])),
        ('interval_9', Pipeline([
            ('extract', FunctionTransformer(feature_interval_9, validate=False)),
            ('scale', StandardScaler()),
            ('shape', ShapeSaver())
         ])),
        ('mon_q1', Pipeline([
...
#        ('is_in_top_site', Pipeline([
#            ('extract', FunctionTransformer(feature_is_in_top_site, validate=False)),
#            ('ohe', OneHotEncoder()),
#            ('shape', ShapeSaver())
#         ])),
...
#x_train_new = x_train_new.drop(sites, axis=1)
#x_test_new = x_test_new.drop(sites, axis=1)
...
c_values = np.arange(0.2, 2.5, step=0.05) # [0.1, 0.05, 2.2, 2.0309, 3.5],
logit_params = {
        'C': np.arange(0.5, 2.5, step=0.1),
...
# + interval_1  0.910397293314 {'C': 2.0309, 'solver': 'lbfgs'}
# interval(1)       0.90840662684 {'C': 3.5, 'solver': 'liblinear'}
# interval(1+2)     0.908656141462 {'C': 3.5, 'solver': 'liblinear'}
# interval(1+2+3)   0.908680180761 {'C': 3.5, 'solver': 'liblinear'}
# interval(1-9)     0.909268927924 {'C': 3.5, 'solver': 'liblinear'}
#                   0.909273542267 {'C': 2.0309, 'penalty': 'l1', 'solver': 'liblinear'}
#   -sites          0.909273542267 {'C': 2.0309, 'penalty': 'l1', 'solver': 'liblinear'}
#                   0.912882091616 {'C': 3.5, 'solver': 'lbfgs'}
#                   0.91297119393 {'C': 2.2000000000000002, 'solver': 'lbfgs'}
#   +uniq_sites(ohe)0.912205728678 {'C': 2.0309, 'solver': 'lbfgs'}
#   +uniq_sites(sc) 0.911908330821 {'C': 2.0309, 'solver': 'lbfgs'} \
#  tsplit=7         0.917249500776 {'C': 2.0309, 'solver': 'lbfgs'}
#  tsplit=7         0.911768848394 {'C': 3.5, 'solver': 'liblinear'}
#                   0.917790153478 {'C': 2.0999999999999996, 'solver': 'lbfgs'}
# tsplit=13+top10   0.905751825686 {'C': 2.0309, 'solver': 'lbfgs'}
# tsplit=12+top10   0.906664879722 {'C': 2.2, 'solver': 'lbfgs'}
# tsplit=12+top3    0.904466803548 {'C': 2.0309, 'solver': 'lbfgs'}
#  site1 in top1    0.912054287983 {'C': 2.0309, 'solver': 'lbfgs'} /!!!
#  site1-3 in top3  0.912054287983 {'C': 2.0309, 'solver': 'lbfgs'}
#  not top          0.91297119393 {'C': 2.2, 'solver': 'lbfgs'}
#                   0.91297119393 {'C': 2.2, 'solver': 'lbfgs'}
#  mon_q 1-4        0.91161897565 {'C': 2.0309, 'solver': 'lbfgs'}
#  mon_q 3-4        0.910845365768 {'C': 2.2, 'solver': 'lbfgs'}
#  mon_q 2,4        0.912434766187 {'C': 2.2, 'solver': 'lbfgs'}
#  mon_q 1          0.912318230562 {'C': 2.2, 'solver': 'lbfgs'}
#  mon_q 2          0.912228877971 {'C': 2.0309, 'solver': 'lbfgs'}
#  mon_q 3          0.910595577509 {'C': 2.0309, 'solver': 'lbfgs'}
#  mon_q 4          0.911752798645 {'C': 2.0309, 'solver': 'lbfgs'}
#  mon_q 1,4        0.913614061326 {'C': 2.2, 'solver': 'lbfgs'}
#      --           0.914326927627 {'C': 2.0999999999999996, 'solver': 'lbfgs'}
#      --           0.908285616377 {'C': 2.3999999999999995, 'solver': 'liblinear'}
...
#  +year_mon        0.909082838729 {'C': 2.0309, 'solver': 'lbfgs'} \
#  +log1p(year_mon) 0.907210689693 {'C': 2.0309, 'solver': 'lbfgs'} \\
...
write_to_submission_file(logit_test_pred, 'no_ym_fg_ts7.csv')
